...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    if (existingUser.length > 0) {
      ...
      return res.status(201).json({ id: result.insertId, user_name, email });
  } catch (err) {
      console.error(err);
      return res.status(500).json({ error: 'Internal Server Error' });
  }
...
exports.login = async (req, res) => {
    ...
    try {
      ...
      const token = jwt.sign({ id: user.id, user_name: user.user_name, email: user.email }, process.env.JWT_SECRET);
